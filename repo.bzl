...
def get_python_path(ctx):
    # NOTE: remove PYTHON_BIN_PATH to get python from virtualenv
...
def _find_python_solib_path(repo_ctx):
    ...
    print("find python solib with basename {}".format(basename))
    exec_result = repo_ctx.execute(
...
def _tensorflow_solib_repo_impl(repo_ctx):
    ...
    if repo_ctx.os.name == "mac os x":
        src = 'libtensorflow_framework.2.dylib'
    else:
        src = 'libtensorflow_framework.so.2'
    print("find tensorflow solib with name {}".format(src))
    repo_ctx.file(
        "BUILD",
        content = """
cc_library(
    name = "framework_lib",
    srcs = ["tensorflow_solib/{}"],
...
def _reverb_protoc_archive(repo_ctx):
    version = repo_ctx.attr.version
    sha256 = repo_ctx.attr.sha256

    override_version = repo_ctx.os.environ.get("REVERB_PROTOC_VERSION")
    if override_version:
        print("Override protoc version as {} from {}".format(override_version, version))
        sha256 = ""
        version = override_version

    if repo_ctx.os.name == "mac os x":
        url = "https://github.com/protocolbuffers/protobuf/releases/download/v%s/protoc-%s-osx-x86_64.zip" % (version, version)
        sha256 = ""
    else:
        url = "https://github.com/protocolbuffers/protobuf/releases/download/v%s/protoc-%s-linux-x86_64.zip" % (version, version)

    print("Fetching protoc from '{}' with sha '{}', osname: '{}'".format(url, sha256, repo_ctx.os.name))
